...
async function renderArchivos() {
    ...
    // Cargar tipos de soporte activos desde la API
    let tiposSoporteOptions = '<option value="">Seleccionar tipo...</option>';
    ...
    try {
        ...
        // Si falla, usar opciones por defecto
        tiposSoporteOptions += '<option value="SOPORTE">Soporte</option>';
        tiposSoporteOptions += '<option value="DISTRIBUCION_GASTO">Distribución de Gasto</option>';
        tiposSoporteOptions += '<option value="OTRO">Otro</option>';
    }
    ...
    // Seleccionar automáticamente FACTURA para el primer archivo
    const primerSelect = document.getElementById('tipo-0');
    if (primerSelect) {
        primerSelect.value = 'FACTURA';
    }
}
                ...
                <div class="table-container">
                    <table class="table">
                        ...
                        <tbody id="tiposSoporteTableBody">
                            <tr><td colspan="3" class="text-center">Cargando tipos de soporte...</td></tr>
                        </tbody>
                    </table>
                </div>
            </div>
...
async function cargarTiposSoporte() {
    ...
    try {
        ...
        if (tipos.length === 0) {
            tbody.innerHTML = '<tr><td colspan="3" class="text-center">No hay tipos de soporte registrados</td></tr>';
            return;
        }
        ...
        document.getElementById('tiposSoporteTableBody').innerHTML =
            '<tr><td colspan="3" class="text-center" style="color: var(--color-error)">Error al cargar datos</td></tr>';
        ...
        <form id="formTipoSoporte">
            <div class="form-group">
                ...
                <input type="text" id="nombre" class="form-input" 
                       ...
                       required 
                       maxlength="100"
                       placeholder="Ej: Factura, Contrato, RUT, etc.">
            </div>
...
async function guardarTipoSoporte(tipoId) {
    ...
    // Auto-generar código desde el nombre (mayúsculas, sin espacios, sin acentos)
    const codigo = nombre
        .toUpperCase()
...
// ============================================
// CONTROL DE VISIBILIDAD DE MENÚ
// ============================================

/**
 * Actualizar visibilidad de opciones de menú según roles
 */
function updateNavigation() {
    ...
    const navUsuarios = document.getElementById('navUsuarios');
    const navProveedores = document.getElementById('navProveedores');
    const navTiposSoporte = document.getElementById('navTiposSoporte');

    // Búsqueda avanzada: SUPER_ADMIN o BUSQUEDA_FACTURAS
    if (navBusqueda) {
        navBusqueda.style.display = (roles.includes('SUPER_ADMIN') || roles.includes('BUSQUEDA_FACTURAS')) ? 'block' : 'none';
    }

    // Usuarios y Proveedores: solo SUPER_ADMIN
    if (navUsuarios) {
        navUsuarios.style.display = roles.includes('SUPER_ADMIN') ? 'block' : 'none';
    }
    if (navProveedores) {
        navProveedores.style.display = roles.includes('SUPER_ADMIN') ? 'block' : 'none';
    }

    // Tipos de Soporte: solo SUPER_ADMIN
    if (navTiposSoporte) {
        navTiposSoporte.style.display = roles.includes('SUPER_ADMIN') ? 'block' : 'none';
    }
...
function setupNavigation() {
    ...
    // Conectar botón de cerrar modal
    const modalClose = document.getElementById('modalClose');
    if (modalClose) {
        modalClose.addEventListener('click', hideModal);
    }
